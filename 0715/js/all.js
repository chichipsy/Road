...
$(function(){
    ...
    $("#con_cancel").click(function () { 
        alert("清除成功")
        $("#connect .connect_con .user_input input").val("");
        $("#connect .connect_con .user_input label").css("opacity", "0");
    });
    $("#con_submit").click(function () { 
        var pattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if($("#Con_Email").val().length > 0){
            if(pattern.test($("#Con_Email").val())){
                alert("送出成功");
                $("#connect .connect_con .user_input input").val("");
                $("#connect .connect_con .user_input label").css("opacity", "0");
            }else{
                $("#connect .connect_con .user_input label").text("電子郵件格式不正確");
                $("#connect .connect_con .user_input label").css("opacity", "1");
            }
        }else{
            $("#connect .connect_con .user_input label").text("電子郵件不能為空");
            $("#connect .connect_con .user_input label").css("opacity", "1");
        }
    });
    ...
    $(window).scroll(function () { 
        ...
        console.log(scroll);
        if(scroll >= 3690){
            $("#connect .connect_con .txt h1 span .line").css("width", "115%");
        }
    });
});